...
function validatePasswordsNumInstances() {
  ...
  let result = input.reduce((runningTotal, line, index) => {
    ...
    // LOL, my initial solution was to use regex, but this isn't strict enough,
    // so it misses the upper bound requirement.
    // const regex = new RegExp(`(.?${requiredCharacter}.?){${numOccurances}}`, 'g');
    // const conforms = regex.test(password);
...
function validatePasswordsBasedOnIndex() {
  console.log("original input length: " + input.length);
  // [positions, requiredCharacter, password]
  let result = input.reduce((runningTotal, line, index) => {
