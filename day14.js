let input = `mask = X00X0X0110010001010X0000000010001111
mem[36604] = 2508973
mem[14290] = 80919
mem[6865] = 15237
mem[45983] = 393572789
...
function part1() {
  ...
  for (let i = 0; i < inputArr.length; i++) {
    ...
    // console.log(`maskCharArr: ${maskCharArr}`);
    // pad binary arr with 0's
    let pad = maskCharArr.length - binaryArr.length;
    for (let p = 0; p < pad; p++) {
      binaryArr.unshift(0);
    }
    // console.log(`padded binaryArr: ${binaryArr}`);

    for (let i = 0; i < maskCharArr.length; i++) {
      if (maskCharArr[i] != 'X') {
        // console.log(`overriding at ${i}, writing ${maskCharArr[i]}`);
        binaryArr[i] = parseInt(maskCharArr[i]);
      }
    }
    // console.log(`new binaryArr: ${binaryArr}`);
    // console.log(`changing ${decimal} to ${parseInt(binaryArr.join(''), 2)}`)
    resultArr[memAddress] = binaryArr.join('');
    // console.log(`resultArr: ${resultArr}`);
    // console.log("*******************")
  }

  let trial = resultArr.slice();
  trial = trial.map(el => parseInt(el, 2));
  let result = trial.reduce((accu, newV) => {
    if (newV) {
      return accu + newV;
    } else {
      return accu;
...
function part2() {
  // LOL I had this as an array before, but that was SO DUMB.
  let resultArr = {};
  let mask = '';
  ...
  for (let i = 0; i < inputArr.length; i++) {
    ...
    for (let i = 0; i < numCopies; i++) {
      ...
      // console.log(`new choice as arr: ${newChoice}`)
      // pad to be the size we care about.
      let pad = binaryArr.length - newChoice.length;
      for (let p = 0; p < pad; p++) {
        newChoice.unshift(0);
      }
      ...
      for (let j = binaryArr.length; j >= 0; j--) {
        // console.log(`looking at index ${j}, value: ${modifiedBinaryArr[j]}`)
        if (binaryArr[j] == 'X') {
      ...
      // console.log(`padded new choice: ${newChoiceArr}`)

      let modifiedBinaryArr = binaryArr.slice();
      // console.log(`modifiedBinaryArr: ${modifiedBinaryArr}`)
      ...
      // console.log(`modifiedBinaryArr2: ${modifiedBinaryArr}`)
      newMemoryAddresses.push(modifiedBinaryArr.join(''));
    }

    // console.log(`newMemoryAddresses: ${newMemoryAddresses}`);
    decMemoryAddresses = newMemoryAddresses.map(el => parseInt(el, 2));
    console.log(decMemoryAddresses)
    // console.log(decimalToSave)
    // console.log(`decMemoryAddresses: ${decMemoryAddresses}`);
    for (let i = 0; i < decMemoryAddresses.length; i++) {
      resultArr[decMemoryAddresses[i]] = decimalToSave;
    }
  }
  let keys = Object.keys(resultArr);
  let total = 0;
  for (let i = 0; i < keys.length; i++) {
    total += resultArr[keys[i]]
  }
  console.log(total)
}
